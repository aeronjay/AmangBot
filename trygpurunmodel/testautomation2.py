...
def run_test_automation(
    ...
    print(f"Starting test automation... Saving to {output_path}")
    ...
    for i, item in enumerate(questions):
        ...
        # Support both 'answer' (Test.json) and 'ground_truth_answer' (100qds.json)
        ground_truth = item.get('answer', item.get('ground_truth_answer', ''))
        
        # New fields for complexity and query type
        complexity = item.get('difficulty', 'N/A')
        query_type = item.get('category', 'N/A')
        
        if query_type == "unanswerable":
            print(f"Skipping Q{question_id}: Category is unanswerable")
            continue
        ...
        # Token count calculation (Question Tokens)
        token_count = 0
        if evaluator.available and hasattr(evaluator, 'tokenizer'):
            token_count = len(evaluator.tokenizer.encode(question_text))
        else:
            token_count = len(question_text.split())
        ...
        results.append({
            ...
            "Response Latency (Seconds)": round(duration, 2),
...
if __name__ == "__main__":
    ...
    dataset_path = os.path.join(project_root, "Test", "Test.json")
    output_path = os.path.join(base_dir, "automation_results.csv")
    
    # Set the number of questions to process
    try:
        user_input = input("Enter number of questions to run (leave empty for all): ").strip()
        num_questions = int(user_input) if user_input else None
    except ValueError:
        print("Invalid input, running all questions.")
        num_questions = None
